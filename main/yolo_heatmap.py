...
class yolo_heatmap:
    def __init__(self, weight, cfg,  device, method, layer, backward_type, conf_threshold, ratio, show_box, renormalize):
        ...
        colors = np.random.uniform(0, 255, size=(len(model_names), 3)).astype(np)
...
def get_params():
    params = {
        ...
        'cfg': 'cfg/training/ems-yolo.yaml',
        'device': 'cuda:0',
        'method': 'XGradCAM',
...
if __name__ == '__main__':
    ...
    # model('inference/images/gg(57).jpg', 'result')
    model('datasets/defect/images/val', 'result')
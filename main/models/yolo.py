...
class Model(nn.Module):
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if self.traced:
                if isinstance(m, Detect) or isinstance(m, IDetect) or isinstance(m, IAuxDetect) or isinstance(m, IKeypoint) or isinstance(m, YOLOXHead) or isinstance(m, DyHead) or isinstance(m, LADHDetect):
                    break

            if profile:
                c = isinstance(m, (Detect, IDetect, LADHDetect, IAuxDetect, IBin))
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
                 ...
                 SwinTransformer2Block, ST2CSPA, ST2CSPB, ST2CSPC, ELAN, SPPCSPCSIM, ELANMRFFI, ConvL, SPPCSPCLight]:
            c1, c2 = ch[f], args[0]
            ...
            c2 = ch[f] // 2
        elif m in [Detect, IDetect, IAuxDetect, IBin, IKeypoint, YOLOXHead, DyHead, LADHDetect]:
...
# 采用解耦检测头（Decoupled Head）结构
import pkg_resources as pkg
def check_version(current='0.0.0', minimum='0.0.0', name='version ', pinned=False, hard=False, verbose=False):
    current, minimum = (pkg.parse_version(x) for x in (current, minimum))
    result = (current == minimum) if pinned else (current >= minimum)  # bool
    s = f'{name}{minimum} required by YOLOv7, but {name}{current} is currently installed'  # string
    if hard:
        assert result, s
    return result
...
class LADH(nn.Module):
    def __init__(self, ch=256, nc=80, anchors=()):
        ...
        self.iou_convs1 = DWConv(ch, 256, 3, 1, 1)
        self.iou_convs2 = DWConv(256, 128, 3, 1, 1)
        self.iou_convs3 = DWConv(128, 64, 3, 1, 1)
        self.obj_convs2 = Conv(ch, 256, 3, 1, 1)
    ...
    def forward(self, x):
        q1 = self.cls_1(x)
        x1 = self.cls_preds(q1)
        p1 = self.iou_convs3(self.iou_convs2(self.iou_convs1(x)))
        x21 = self.reg_preds(p1)
        x2 = self.obj_convs2(x)
...
class LADHDetect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):
        ...
        self.m = nn.ModuleList(LADH(x, nc, anchors) for x in ch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='cfg/training/yolov7-tiny-ELANMO.yaml', help='model.yaml')

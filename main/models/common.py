...
class ConvL(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        # self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.conv = DWConv(c1, c2, k, s, act)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
...
class SPPCSPCLight(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        super(SPPCSPCLight, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        # self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        # self.m = nn.ModuleList([nn.MaxPool2d(kernel_size=x, stride=1, padding=x // 2) for x in k])
        ...
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
        #测试添加注意力
        self.sa_attention = ShuffleAttention(c2)
    ...
    def forward(self, x):
        ...
        x7 = self.cv3(x6)
        return self.sa_attention(self.cv4(torch.cat((x2,x7),1)))

class ELANMRFFI(nn.Module):
    # Yolov7 ELAN with args(ch_in, ch_out, kernel, stride, padding, groups, activation)
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, global_ratio=0.8,
                 local_ratio=0.2, act=True):
        super().__init__()
        c_ = int(c2 // 2)
        c_out = c_ * 4
        self.cv1 = Conv(c1, c_, k=k, s=s, p=p, g=g, act=act)
        ...
        self.cv4 = Conv(c_, c_, k=3, s=s, p=p, g=g, act=act)
        self.cv5 = Conv(c_out, c2, k=k, s=s, p=p, g=g, act=act)
        self.cv6 = MambaModule(dim=c2, global_ratio=global_ratio, local_ratio=local_ratio)
    ...
    def forward(self, x):
        ...
        x5 = torch.cat((x1, x2, x3, x4), 1)
        # return self.cv5(x5)
        x6 = self.cv6(self.cv5(x5))
        return self.triplet_attention(x6)
...
#############ShuffleAttention注意力机制（ShuffleAttention）#############
...
from .MambaNet.lightweight_mamba import DWConv2d_BN_ReLU,MBWTConv2d,Conv2d_BN,nearest_multiple_of_16

class MambaModule(torch.nn.Module):
    def __init__(self, dim, global_ratio=0.8, local_ratio=0.2,
                 kernels=3, ssm_ratio=2, forward_type="v052d",):
        super().__init__()
        self.dim = dim

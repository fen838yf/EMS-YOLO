...
from utils.general import bbox_iou, bbox_alpha_iou, box_iou, box_giou, box_diou, box_ciou, xywh2xyxy, bbox_mpdiou
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # mpdiou
                iou = bbox_mpdiou(pbox.T, tbox[i], x1y1x2y2=False, mpdiou_hw=pi.size(2) ** 2 + pi.size(3) ** 2,
                                  grid=torch.stack([gj, gi]))  # iou(prediction, target)
                lbox += (1.0 - iou).mean()  # iou loss
                # 更换损失函数NWD
                # nwd = wasserstein_loss(pbox, tbox[i])
                # iou_ratio = 0.7  #iou_ratio是超参，可以调，如果小目标比较多的话，这个值可以低一些，从而增加nwd损失的比例
